# EmblemCon  

EmblemCon is a free digital Discord convention made by Fire Emblem fans, for Fire Emblem fans.  

## Installation  

1. Click the green "code" button on Github  
2. Click 'Download as ZIP'  
3. Name it and save it.  
4. Locate where you saved it.  
5. If not automatically extracted: Right click and Extract (download WinRar if no extractor is available)  
6. Extract all files.  
7. Find the extracted folder and open it :D    

## Usage  

This project is meant to be used by Delilah Singer/Lightning, Rhea Hyde/DucessaEva, the EmblemCon mod team, and related parties only.   

## Contributing  

If you wish to contribute to EmblemCon, please fill out forms on the [applications page](apply.html) when they are open or keep an eye out on the [site updates](announcements.html) page for ways to become part of the convention.  

## Credits  

All images used are used for a transformative work and nonprofit. The images are copyrighted or is a registered trademark. The contributor claims fair use. No copyright infringement is intended.  

Certain materials are included under fair use exemption of the U.S. Copyright Law and are restricted from further use.  

© 2026 Nintendo / INTELLIGENT SYSTEMS  

## License  

MIT- please see [license file](LICENSE).  